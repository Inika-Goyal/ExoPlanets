...
@app.post("/upload")
async def upload_file(background_tasks: BackgroundTasks, file: UploadFile = File(...), baseline: UploadFile | None = None):
    ...
    data = await utils.read_uploaded_file(file)
    baseline_data = None
    if baseline:
        baseline_data = await utils.read_uploaded_file(baseline)
